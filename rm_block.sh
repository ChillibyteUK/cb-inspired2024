...
# Get all block files
blocks_dir="./blocks/"
block_files=($(ls -1 ${blocks_dir}cb_*.php 2>/dev/null | sort))
...
base_slug=$(echo "$block_slug" | sed 's/^cb_//')
block_name=$(echo "$base_slug" | sed 's/_/ /g' | awk '{for(i=1;i<=NF;i++) $i=toupper(substr($i,1,1)) tolower(substr($i,2));}1')
...
foreach ($posts as $post) {
    if (strpos($post->post_content, 'wp:acf/' . $block_slug) !== false || strpos($post->post_content, 'wp:acf/' . $block_kebab) !== false) {
...
if [ -f "$blocks_scss" ] && grep -q "@import '${base_slug}';" "$blocks_scss"; then
  temp_file=$(mktemp)
  grep -v "@import '${base_slug}';" "$blocks_scss" > "$temp_file"
...
if [ $removed_count -gt 0 ]; then
  echo ""
  echo -e "${GREEN}Block removed successfully.${NC}"
fi

if [ ${#not_found[@]} -gt 0 ]; then
  echo ""
  echo -e "${YELLOW}Some items were not found: ${not_found[*]}${NC}"
fi
