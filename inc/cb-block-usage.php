...
function block_usage_table_shortcode() {
	...
	// Extract block names from filenames (cb_block_name.php -> cb_block_name).
	$block_names = array();
	...
	foreach ( $posts as $post ) {
		...
		// ACF blocks are stored as: <!-- wp:acf/cb_block_name {...} /-->.
		preg_match_all( '/<!-- wp:acf\/(cb[_-][a-z0-9\-_]+)\s/', $content, $matches );
